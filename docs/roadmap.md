...
- ✅ 已落地（v1.22.0）：**数据库迁移一致性校验**。`php artisan db:check`（别名 `composer db:check`）比对文件系统迁移与 `migrations` 表，存在 Pending 则非零退出；接入 `composer setup` 与 CI 的 MySQL 矩阵（新增 `db:check` 步骤）。CI 同时扩展为 `[sqlite, mysql]` 双引擎，覆盖「测试用 SQLite 全过、生产 MySQL 缺列 500」这类 schema 脱节事故。
